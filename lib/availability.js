const { jalaaliMonthLength, dateFromJalaaliDateTime, toJalaali, instantToTehranParts } = require('./jalaali');
...
function computeRangeAvailability({ days, confirmedBookings = [], blockedSlots = [], now = new Date() }) {
  const result = [];
  // "Today" means today in Asia/Tehran, not the UTC calendar date the server
  // host happens to be on — matters near local midnight (see docs/AUDIT.md
  // finding #1). `today` below is a UTC-midnight marker standing in for that
  // Tehran calendar day; re-deriving its Jalali y/m/d per iteration and
  // routing through dateFromJalaaliDateTime (rather than re-deriving slot
  // instants inline) keeps this in lockstep with every other slot-boundary
  // computation in the codebase.
  const nowTehran = instantToTehranParts(now);
  const today = new Date(Date.UTC(nowTehran.year, nowTehran.month - 1, nowTehran.day));

  for (let i = 0; i < days; i++) {
    const day = new Date(today.getTime() + i * 24 * 60 * 60 * 1000);
    const j = toJalaali(day.getUTCFullYear(), day.getUTCMonth() + 1, day.getUTCDate());
    const slots = TIME_SLOTS.map((time) => {
      const [hh, mm] = time.split(':').map(Number);
      const slotStart = dateFromJalaaliDateTime(j.jy, j.jm, j.jd, hh, mm);
      return { time, open: isSlotOpenAt(slotStart, confirmedBookings, blockedSlots, now) };
    });

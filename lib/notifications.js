const { toJalaali, shamsiMonths, weekdayNameFa, instantToTehranParts } = require('./jalaali');
...
function formatJalaaliDateTime(startAt) {
  const { year, month, day, hours, minutes } = instantToTehranParts(startAt);
...
/** Day-group header for /calendar, e.g. "شنبه ۱۴ شهریور" (weekday + day + month, no year/time — matches the command's grouped-by-day layout). */
function formatJalaaliDayHeader(startAt) {
  const { shifted, year, month, day } = instantToTehranParts(startAt);
  const j = toJalaali(year, month, day);
  return `${weekdayNameFa(shifted)} ${toFaDigits(j.jd)} ${shamsiMonths[j.jm - 1]}`;
}

/** Time-only portion of formatJalaaliDateTime, e.g. "۱۴:۰۰" — for /calendar's per-booking lines, where the date is already in the day header. */
function formatJalaaliTime(startAt) {
  const { hours, minutes } = instantToTehranParts(startAt);
  const hh = String(hours).padStart(2, '0');
  const mm = String(minutes).padStart(2, '0');

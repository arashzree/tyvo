...
- **10e.** The whitelist gate itself (`telegram-webhook.js:19-30`) needs an exception: it currently replies "این بات خصوصی است" to *any* message from a non-whitelisted chat, and the group's chat_id will never be individually whitelisted, so ordinary chatter in the group — not just join/leave events — would keep triggering that reply. Add a check: if `ctx.chat.id === TYVO_GROUP_ID`, return silently instead of replying. This is narrower than "skip the gate for this chat" — a stray command typed in the group still shouldn't do anything privileged, it just shouldn't get a reply either.
- **10f. Group migration (`migrate_to_chat_id`).** `TYVO_GROUP_ID` is a Netlify env var — static, deploy-time config the running function cannot update itself. If Telegram ever migrates the tracked group from a basic group to a supergroup (confirmed: `-4295519690`, captured via `/chatid`, is a basic-group id, not the `-100...` supergroup form), the group gets a *new* chat_id and every future event arrives under that new id — silently breaking sync, since nothing currently distinguishes "no events happening" from "events happening under an id we stopped recognizing." A bare log line isn't a sufficient fix on its own: function logs aren't reliably checked by either of us (Netlify CLI log access has returned 403 all session). Two-part fix instead:
  - Detect `ctx.message.migrate_to_chat_id` specifically when it arrives in the currently-tracked `TYVO_GROUP_ID` chat, and DM every `owner` immediately with the new id and the exact env var to update — turns a silent, delayed-discovery failure into an immediate, actionable one, without needing TYVO_GROUP_ID to become dynamically self-updating (a bigger change: moving it out of an env var into a DB-backed setting, not justified for an event this rare).
  - As a general safety net beyond migration specifically (e.g., bot mistakenly added to a second group), log a warning whenever a `new_chat_members`/`left_chat_member` event arrives from a group chat_id that isn't `TYVO_GROUP_ID` — cheap, catches other misconfiguration even though it doesn't self-heal.

Confirmed live (2026-09-08, 11:17): the bot is already in the group as admin and the webhook is receiving group messages — it replied "این بات خصوصی است" to a group message, which is exactly the noise 10e silences.

Confirmed live (2026-09-09): `TYVO_GROUP_ID=-4295519690` (TyvoRentalSup, captured via a temporary `/chatid` command) is set in Netlify across all contexts, pending redeploy. Sinajulay2 is already a member of that group from before any sync code exists — `new_chat_members` won't fire retroactively for them, so they'll need a one-time `/addmember` or to leave/rejoin once 10a-10c ship.
